...
/*   Updated: 2020/08/01 22:21:07 by zgargasc      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	check_light(t_ray **ray, t_data *mlx, t_obj_list **list)
{
	...
	while (current)
	{
		dist = vector_sub(&current->t_light.light_p, &hit_p);
		...
		check_hit_l(ray, list, sqrt(vector_dot(&dist, &dist)));
		if ((*ray)->hit.check == 0)
